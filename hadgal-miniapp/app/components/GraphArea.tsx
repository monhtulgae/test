...
export default function GraphArea({ interest }: NumberProps) {
  ...
  return (
    <div className="my-2 bg-white p-3 rounded-xl">
      {interest === -1 ? (
        <div className="mb-3 text-xl">Нийт хандивласан: {amount}</div>
      ) : (
        <>
          <div className="mb-3 text-lg">Нийт хөрөнгө оруулалт: {amount}</div>
          <div className="mb-3 text-lg">Нийт ашиг: {interest}</div>
        </>
      )}
      <hr />
      <div className="">
        <p className="text-lg">{interest === -1 ? "Хандивласан төсөл" : "Хөрөнгө оруулсан төсөл"}</p>
